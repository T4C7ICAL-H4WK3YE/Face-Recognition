...
if not os.path.exists('face_data.xlsx'):
    # Create an empty DataFrame
    empty_df = pd.DataFrame(columns=['Name', 'Timestamp'])
    # Write the empty DataFrame to an Excel file
    empty_df.to_excel('face_data.xlsx', index=False)
    ...
    for face_encoding, face_location in zip(face_encodings, face_locations):
        ...
        if True in matches:
            ...
            if name not in recognized_face_info:
                recognized_face_info[name] = {'timestamp': datetime.now()}
            ...
            if name != 'Unknown':
                update_excel(name, recognized_face_info[name]['timestamp'])
        else:
...
def update_excel(name, timestamp):
    # Read existing data from Excel file
    existing_data = pd.read_excel('face_data.xlsx')
    ...
    if name.strip() and name not in existing_data['Name'].tolist():
        ...
        dataframe = pd.DataFrame([[name, timestamp]], columns=['Name', 'Timestamp'])
        updated_data = pd.concat([existing_data, dataframe], ignore_index=True)
        updated_data.to_excel('face_data.xlsx', index=False)
